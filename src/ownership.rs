...
impl SharedObjectManager {
    ...
    pub fn verify_shared_object_access(
        ...
        debug!("Verifying shared object access: object={}", object.id);

        self.verify_is_shared(object)?;

        // Shared objects can be accessed by anyone
        debug!(
            "Access granted: shared object {} can be accessed by any transaction",
            object.id
        );
        Ok(())
    }
    ...
    pub fn make_shared(
        ...
        debug!(
            ...
            shared_object.id, shared_object.version, shared_object.version
        );
...
impl ImmutableObjectManager {
    ...
    pub fn validate_immutable_object_input(&self, object: &Object) -> Result<()> {
        debug!("Validating immutable object input: object={}", object.id);
...
impl WrappedObjectManager {
    ...
    pub fn get_parent_object(&self, object: &Object) -> Option<ObjectID> {
        debug!("Getting parent object for wrapped object: {}", object.id);
...
impl OwnershipTransferManager {
    ...
    pub fn batch_transfer_ownership(
        ...
        debug!("Batch transferred {} objects successfully", results.len());
    ...
    pub fn get_transfer_history(&self, _object_id: &ObjectID) -> Vec<OwnershipTransferEvent> {
        // Transfer history would be queried from the event store
        // For now, return empty vector as this is a placeholder for event store integration
        // In production, this would query CF_EVENTS for transfer events related to the object
        Vec::new()
    }
}
